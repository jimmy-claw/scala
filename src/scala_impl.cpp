...
ScalaImpl::ScalaImpl() {
    ...
    // NOTE: kv_module not available at construction — loaded in onContextReady
}
...
void ScalaImpl::onContextReady() {
    ...
    // At this point, modules().kv_module is available for inter-module calls.
    //
    // In the universal pattern, this replaces initLogos(LogosAPI*).
    // The generated glue calls onContextReady after wiring up dependencies.
    //
    // TODO: Wire up kv_module via modules().kv_module when the typed SDK
    // is generated by logos-cpp-generator. For now, CalendarStore uses
    // in-memory fallback (same as standalone mode).

    // Load stored identity or generate stable fallback
    std::string stored = m_store->kvGet("identity");
    if (!stored.empty()) {
        m_identity = stored;
    } else {
    ...
    // Set kv_module data dir for file backend persistence
    // TODO: modules().kv_module.setDataDir(...) when typed SDK is available
}
...
void ScalaImpl::setIdentity(const std::string& pubkeyHex) {
    if (m_identity != pubkeyHex) {
        ...
        m_store->kvSet("identity", std::string(m_identity));
...
bool ScalaImpl::deleteCalendar(const std::string& id) {
    ...
    if (m_sync->isSyncing(QString::fromStdString(id)))
        m_sync->stopSync(QString::fromStdString(id));
...
std::string ScalaImpl::createEvent(const std::string& calendarId, const std::string& eventJson) {
    ...
    if (m_sync->isSyncing(ev.calendarId)) {
        ...
        msg.senderId = QString::fromStdString(m_identity);
...
std::string ScalaImpl::shareCalendar(const std::string& calendarId) {
    ...
    m_sync->startSync(cal.id, cal.encryptionKey);
...
bool ScalaImpl::joinSharedCalendar(const std::string& calendarId,
    ...
    m_sync->startSync(qCalId, qKey);

    // Request bulk sync from peers
    SyncMessage msg;
    msg.type = SyncMessageType::SyncEvents;
    msg.calendarId = qCalId;
    msg.senderId = QString::fromStdString(m_identity);
    msg.timestamp = QDateTime::currentMSecsSinceEpoch();
    m_sync->sendMessage(qCalId, msg);
...
void ScalaImpl::onSyncMessageReceived(const std::string& calendarId, const std::string& msgJson) {
    ...
    case SyncMessageType::CreateEvent: {
        QJsonDocument evDoc = QJsonDocument::fromJson(msg.payload.toUtf8());
    ...
    case SyncMessageType::SyncEvents: {
        ...
        for (const auto& ev : events) {
            ...
            reply.calendarId = qCalId;
            reply.senderId = QString::fromStdString(m_identity);
            reply.payload = QString::fromUtf8(
                QJsonDocument(ev.toJson()).toJson(QJsonDocument::Compact));
            reply.timestamp = QDateTime::currentMSecsSinceEpoch();
            m_sync->sendMessage(qCalId, reply);
        }
        break;
    }
    case SyncMessageType::UnshareCalendar: {
        m_sync->stopSync(qCalId);
        auto cal = m_store->getCalendar(qCalId);
        if (!cal.id.isEmpty()) {
            cal.isShared = false;
